...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const gip1 = Math.sqrt(x1 * x1 + y1 * y1);
  const gip2 = Math.sqrt(x2 * x2 + y2 * y2);
  const cos = mul / (gip1 * gip2);
  const fi = Math.acos(cos);
  return fi;
}
...
function isPrime(n) {
  ...
  for (let i = 2; i <= sq; i += 1) if (n % i === 0) return false;
  return true;
}
...
function getSumOfDigits(num) {
  return `${num}`.split('').reduce((acc, val) => acc + parseInt(val, 10), 0);
}
